				...
				<c:if test="${empty allCatalog}">
					<div class="box">
						<c:choose>
							...
							<c:otherwise>
								<h1>
									<spring:message code="emptyCatalog" />
								</h1>
							</c:otherwise>
						</c:choose>
					</div>
						...
						<div class="col-md-3 col-sm-4">
							<div class="product">
								...
								<div class="text">
									<h3>
										<a
											href='<spring:url value="\rocknshop/details/${model.idModel}"/>'>${model.nameModel}</a>
									</h3>
											...
											<c:when test="${not empty promotionList}">
												...
												<c:forEach items="${promotionList}" var="promo">
													<c:if test="${!priceHasBeenShown}">
														<c:choose>
															<c:when
																test="${promo.brand.nameBrand == model.brand.nameBrand}">

																<fmt:formatNumber
																	value="${model.price * (1-promo.percentageReduction)}"
									...
									<p class="buttons">
										<a
											href='<spring:url value="\rocknshop/details/${model.idModel}"/>'
											class="btn btn-default"><spring:message code="viewDetail" /></a>
										<a
